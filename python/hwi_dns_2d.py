...
# Background shear
u['g'][0] = np.tanh(z)
# Background density stratification
# if rho_ampl != 0:
#     rho.fill_random('g', seed=42, distribution='normal', scale=rho_ampl)
#rho['g'] *= np.exp(-z**2)
...
A['g'] *= 1 - (2*z/Lz)**2 # Damp noise at walls

up = d3.skew(d3.grad(A)).evaluate()
up.change_scales(1)
u['g'] += 1e-3*up['g'] 

#u['g'][1] += 0.1 * np.sin(2*np.pi*x/Lx) * np.exp(-z**2)
...
snapshots = solver.evaluator.add_file_handler(datadir/'snapshots', sim_dt=0.05, max_writes=10)
